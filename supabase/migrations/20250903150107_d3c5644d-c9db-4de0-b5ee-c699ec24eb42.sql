...
-- Modify the user_id column to allow string values for anonymous users

-- First, update the column type to allow text instead of uuid for anonymous enrollments
ALTER TABLE public.enrollments ALTER COLUMN user_id TYPE text;

-- Update the RLS policy for enrollments to handle both authenticated and anonymous users
